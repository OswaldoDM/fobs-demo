...
function BusinessCard({
  ...
  return (
    <div className={`
      ${cardType === 'slider' ? 'flex-none w-full sm:w-[47.6%] md:w-[48%] lg:w-[31.8%] xl:w-[23.8%] 2xl:w-[24%] overflow-hidden animate-slideIn motion-reduce:animate-none' : ''} 
      ${background === 'gray' ? 'bg-primary_gray' : 'bg-black text-primary_gray'} text-xs rounded-lg border border-secondary_gray
      `}>
      ...
      <div className='relative h-[300px] md:h-[240px] overflow-hidden'>
        ...
        />
        <div className='bg-[#181818] rounded py-1 px-2 absolute top-1 left-1'>
          {typeLocation}
        </div>
      </div>            
      ...
      <div className="flex flex-col justify-between gap-3 py-3 px-2 2xl:min-h-[50%] 2xl:py-4">
        ...
        <div className='flex gap-2 flex-wrap'>
          {tags.map((tag, tagIndex) => (
            <div
              key={tagIndex}
              className={`border-[0.5px] ${background === "gray" ? 'border-black' : 'border-primary_gray'} rounded-lg py-1 px-2`}
            >
              {tag}
            </div>
          ))}
